...
def update_user(user_id):
    ...
    try:
        ...
        fname = content['fname']
        lname = content['lname']
        secquestion = content['secquestion']
        secanswer = content['secanswer']
    except:
        return json.dumps({"message": "error reading arguments"})
    url = db['name'] + '/' + db['endpoint'][3]
    response = requests.put(url, params = {"objtype": "user", "objkey": user_id}, json = {"uname":uname, "fname":fname, "lname": lname, "secquestion": secquestion, "secanswer": secanswer})
...
def create_user():
    ...
    response = requests.post(url, json = {"objtype":"user","uname":uname, "fname": fname, "lname": lname, "secquestion":secquestion, "secanswer":secanswer})
    return (response.json())
